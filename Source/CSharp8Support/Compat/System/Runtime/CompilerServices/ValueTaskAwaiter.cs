...
namespace System.Runtime.CompilerServices
{
    ...
    public readonly struct ValueTaskAwaiter : ICriticalNotifyCompletion, IStateMachineBoxAwareAwaiter
    {
        ...
        void IStateMachineBoxAwareAwaiter.AwaitUnsafeOnCompleted(IAsyncStateMachineBox box)
        {
            ...
            if (obj is Task t)
            {
                TaskAwaiterEx.UnsafeOnCompletedInternal(t, box, continueOnCapturedContext: true);
            }
            else if (obj != null)
            {
                Unsafe.As<IValueTaskSource>(obj).OnCompleted(ThreadPoolGlobalsEx.s_invokeAsyncStateMachineBox, box, _value._token, ValueTaskSourceOnCompletedFlags.UseSchedulingContext);
            }
            else
            {
                TaskAwaiterEx.UnsafeOnCompletedInternal(TaskEx.CompletedTask, box, continueOnCapturedContext: true);
            }
        }
    }
    ...
    public readonly struct ValueTaskAwaiter<TResult> : ICriticalNotifyCompletion, IStateMachineBoxAwareAwaiter
    {
        ...
        void IStateMachineBoxAwareAwaiter.AwaitUnsafeOnCompleted(IAsyncStateMachineBox box)
        {
            ...
            {
                Unsafe.As<IValueTaskSource<TResult>>(obj).OnCompleted(ThreadPoolGlobalsEx.s_invokeAsyncStateMachineBox, box, _value._token, ValueTaskSourceOnCompletedFlags.UseSchedulingContext);
            }
            else
            {
                TaskAwaiterEx.UnsafeOnCompletedInternal(TaskEx.CompletedTask, box, continueOnCapturedContext: true);
